...
| P0-2 | **B1/B2 codec / AudioTrack 5001 path** | ✅ **Diagnosed + fixed 2026-06-15 (2):** F5121 sim soak (`bhhk`/`oat9`, apk 34/0.3.5) proved 5001 = **AudioTrack-pool exhaustion** (not codec; `decoder_software=true`, `players_prepared≈13`). **B2b** (reactive reclaim+retry, any player) + **narrowed B2a** (proactive headroom, `PREPARED_SOFT_CAP=11`) shipped in **audio-simple 0.3.6**. **Remaining:** apk 35 re-soak (expect 5001→~0, `audio_track_reclaim` with `reclaimed>0`) → one clean fleet field day → *then* Howler retirement. Structural B2a (eager zone/global release) deferred unless the population still rides the cap. See mobile-audit.md 2026-06-15 (2) addendum. |
