...
| P0-2 | **B1/B2 codec / AudioTrack 5001 path** | ✅ **Diagnosed + fixed 2026-06-15 (2):** F5121 sim soak (`bhhk`/`oat9`, apk 34/0.3.5) proved 5001 = **AudioTrack-pool exhaustion** (not codec; `decoder_software=true`, `players_prepared≈13`). **B2b** (reactive reclaim+retry, any player) + **narrowed B2a** (proactive headroom, `PREPARED_SOFT_CAP=11`) shipped in **audio-simple 0.3.6**. **CLOSED.** apk-35 re-soak (`mtme`/`wdly`) → 0 errors; **2026-06-16 backpack field day** (apk 36/0.3.7, 9 walks, 0 audio errors, F5121 clean) confirmed in the wild — proactive cap rides to 11 then sweeps (1–4×/walk, all `proactive:true`; reactive B2b never fired). **Howler-retirement gate now MET** (clean re-soak + clean fleet day) — unblocked. Structural B2a stays deferred (cap sufficient; only residual = `players_total` up to 38 = instance accumulation, no failure). Media downloader also field-validated same day (slow 6.5-min download, `retried:0`, no partials). See mobile-audit.md 2026-06-16 addendum. |
| P0-3 | **Chunked media downloader field-validation** | ✅ **DONE 2026-06-16:** first real on-site downloads — `uckx` 6m31s for the full 176 MB pack over a ~450 KB/s link, `retried:0`, **zero `media_download_partial`** all day (the old 12 s-timeout failure that killed the 12.4 MB Alex_secours file is closed). |
