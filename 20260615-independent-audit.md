...
| P0-2 | **B1/B2 codec / AudioTrack 5001 path** | ✅ **Diagnosed + fixed 2026-06-15 (2):** F5121 sim soak (`bhhk`/`oat9`, apk 34/0.3.5) proved 5001 = **AudioTrack-pool exhaustion** (not codec; `decoder_software=true`, `players_prepared≈13`). **B2b** (reactive reclaim+retry, any player) + **narrowed B2a** (proactive headroom, `PREPARED_SOFT_CAP=11`) shipped in **audio-simple 0.3.6**. **apk-35 re-soak (`mtme`/`wdly`) VALIDATED:** 0 audio errors on the F5121 (was 17/20) — the proactive cap prevented all 5001s (reactive B2b never fired). **0.3.7** adds proactive-reclaim telemetry (`proactive:true`). **Remaining:** one clean **fleet field day** (real walk) → *then* Howler retirement. Structural B2a deferred unless the 0.3.7 telemetry shows the population pinned at the cap. See mobile-audit.md 2026-06-15 (2) addendum. |
