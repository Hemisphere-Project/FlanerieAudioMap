...
L.TileLayer.Cordova = L.TileLayer.extend({
    ...
    downloadAndStoreTile: function (x, y, z, success_callback, error_callback) {
        ...
        // Download tile and write to local filesystem
        fetchRemote(sourceurl)
            .then(response => response.blob())
            .then(blob => {
                // dirhandle is the DirectoryEntry for the tile cache folder
                return app_write(myself.dirhandle, blob, filename);
            })
            .then(() => {
                if (success_callback) success_callback();
            })
