...
function load() {
    return PARCOURS.load(parcoursID)
            .then(() => {
                ...
                if ('zones' in PARCOURS.spots) 
                    for (let i = 0; i < PARCOURS.spots.zones.length; i++) {
                        const z = PARCOURS.spots.zones[i]
                        const zone = z._spot

                        const li = $('<li class="list-group-item spots-edit zones-edit">')
                        li.appendTo('#pZones').click(() => z.select().center())
                        ...
                        // body: audio select 
                        const body = $('<div>').addClass('edit-body').appendTo(li)
                        body.append($('<span>').addClass('badge me-1').addClass(zone.mode == 'Ambiance' ? 'bg-success' : 'bg-info').text(i))
                        ...
                        // buttons
                        body.append($('<button>').addClass('btn btn-sm btn-danger btn-sm float-end p-1 me-1').html('<i class="bi bi-trash"></i>').click(() => {
                            if (confirm('Supprimer ' + z.name() + ' ?')) PARCOURS.deleteSpot('zones', i) && save()
                        }))
                        body.append($('<button>').addClass('btn btn-sm btn-info btn-sm float-end p-1 me-1').html('<i class="bi bi-arrow-up"></i>').click(() => {
                            PARCOURS.moveSpot('zones', i, i - 1) && save().then(()=> PARCOURS.select('zones', i - 1))
                        }))
                        body.append($('<button>').addClass('btn btn-sm btn-info btn-sm float-end p-1 me-1').html('<i class="bi bi-arrow-down"></i>').click(() => {
                            PARCOURS.moveSpot('zones', i, i + 1) && save().then(()=> PARCOURS.select('zones', i + 1))
                        }))
...
MAP.on('editable:vertex:dragend', function (e) {
    ...
    try {
        ...
        s.lat = spot.getCenterPosition().lat
        s.lon = spot.getCenterPosition().lng
