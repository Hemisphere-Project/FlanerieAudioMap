...
**Original:** 2026-04-27
**Last updated:** 2026-06-19 (first public field day, ~35 walks, apk 36 — clean on audio/build/GPS-blackout, ~34 reached step 20; but the **Android background JS-suspension P0 REGRESSED on the loaner fleet** (SM-A515F/A415F): native bg-geo delivered ~2,600 fixes + 41 rail wakes while the JS position pipeline (`gps` + `route_probe`) sat silent ~44 min — `z23h`/`bt1t`/`gjtm` "completed" via native-audio chaining, not GPS. Also: A54 `qhkz` crash-looped at step 1; iPhone 11 `fgl3` blocked by Motion=Denied (`auth_status:2`, not the bug-2 NotDetermined hang). Phased fix plan — see [2026-06-19 addendum](#2026-06-19-addendum--public-field-day-flanerie_invites-apk-36-android-background-js-suspension-p0-regressed-on-the-loaner-fleet-fix-plan).) Prior: 2026-06-11 (two-wave field day, 14 walks, apk 31 — **Android background JS-suspension P0 VALIDATED at scale and CLOSED**: max JS-event gap ≤ 6.1 s across 10 Android models (8→15) + 2 iPhones over 40–55 min locked-in-pocket; D1 watchdog zero false fires, D2 Android rail 42–48 wakes/walk. New findings: **H1-ter** ExoPlayer codec exhaustion reproduced on audio-simple 0.3.4 (2nd device, F5121), Wi-Fi-only loaners lose walk telemetry (in-memory buffer, no post-end retry), iOS fix-*quality* collapse invisible to the signal-state machine (iPhone 8 missed steps 2–3), `gps_recovered` vibrates for never-escalated episodes, startup-gate evaluator emits past walk start. Fix plan rounds A (webapp) + B (audio-simple native) — see [2026-06-11 addendum](#2026-06-11-addendum--two-wave-field-day-toussieuflanerie_invites_v3-android-js-suspension-p0-validated-at-scale-4-new-findings--fix-plan).) Prior: 2026-06-10 (full-codebase review + **keepalive-stack hardening #1–#5 + tier-1.5 auto-foreground** — D1 watchdog now survives a process kill mid-walk (persisted walk gate), GPS blackouts can't false-fire it (timer ack + renderer probe), lever 1 re-pokes from the fix path + its asset is now an inaudible 20 Hz tone (defeats Chromium's power-based audibility check), and loan phones with the overlay grant self-recover from a freeze with no walker tap (`WatchdogRecoveryActivity`, power-opt PO-11, devmode grant flow); bg-geo v2.15.1-pending + power-opt v0.3.3-pending, same apk 30 — see the addendum's "D1/keepalive hardening", "#5", and "A" sections — see [2026-06-10 addendum](#2026-06-10-addendum--full-codebase-review-startup-gate-simplified-to-a-flat-bar-server-auth-hardening): the 2026-06-09 #2 **adaptive startup relax was found buggy and REPLACED with a flat 20 m bar** (acceptance and the rdv gate now agree by construction; the relax machinery no longer exists in source — read the addendum, not the #2 section); + 3 server-auth fixes. Prior: 2026-06-09 (pre-VILLEURBANNE robustness review — see 2026-06-09 addendum (2): D1 Android native JS-liveness watchdog [approved], D2 Android `GeofencingClient` wake-rail promoted to prototype-now, D3 device-list commitments [SM-A528B codec re-test + weak-GPS], D4 server telemetry corrupt-file hardening [SHIPPED], D5 RESUME cue marked PAUSED. Backpack multi-device test, apk 28 — see 2026-06-09 addendum. **iOS native plan validated end-to-end** on real locked-pocket walks: rail wake 38×, audio-simple under load 0 errors, motion grant. **Android background WebView JS-suspension is a new P0** (likely launch blocker): locked-pocket walks freeze the JS event loop 40+ min while native GPS stays alive — ExoPlayer-migration regression. **Fix SHIPPED** = keepalive levers 1–3 (in-renderer Web Audio + `KeepRunning` + power-opt 0.3.2 renderer-priority) + 7 follow-up fixes; **needs apk 29 build (levers 2–3 are native) + backpack re-test to validate.** See the 2026-06-09 addendum fix-plan + follow-up sections.))
**Scope:** Cordova launcher (FlanerieCordova) + downloaded local webapp (FlanerieAudioMap) + four forked native plugins
**Field tests so far:** ELYSEE (multiple), FRAPPAZ, GUILLOTIÈRE (2024-12), GIVORS (2026-05-20, archived), FRAPPAZ+ST-BONNET (2026-06-05), iOS-GIVORS-V7 (2026-06-06), backpack multi-device (2026-06-09), two-wave 14-walk day incl. 12-phone backpack (2026-06-11, apk 31 — P0 validated), backpack apk-36 (2026-06-16), **first public day (2026-06-19, apk 36 — JS-suspension P0 regressed on loaner fleet)**. Next: VILLEURBANNE.
...
- **Global persistent players:** `SILENT_PLAYER`, `GPSLOST_PLAYER`, `DEFAULT_AFTERPLAY_PLAYER`, `RESUME_PLAYER`, `LOST_PLAYER`, `testplayer`. All load from `www/app/images/{afterplay,resume,youlost,gpslost}.mp3`. Bundled placeholders ship as `_afterplay.mp3` etc. — operator renames to enable. **`RESUME_PLAYER` is a PAUSED feature (2026-06-09 D5): no `resume.mp3` is bundled, every call site is `isLoaded()`-guarded → no-op. Wired but not in use.**
...
| `cordova-plugin-power-optimization` | 0.3.2 | ✅ pinned @ `baaf8a6` (PO-10 `SetRendererPriority` — keep WebView renderer at IMPORTANT priority when hidden; 2026-06-09 keepalive lever 3) |
| `cordova-background-geolocation-plugin` | 2.15.0 (fork; apk 29 carries 2.14.12) | ✅ main-thread `CLLocationManager` singleton fix + iOS stream-health + motion deferral + `start`/`stop` real outcome (2.14.x). **2.15.0 (2026-06-09): D1 Android JS-liveness watchdog + D2 Android `GeofencingClient` wake-rail — code complete, needs apk 30 build.** |
...
- **✅ P0 — Android background WebView JS-suspension — VALIDATED AND CLOSED (2026-06-11).** The keepalive levers 1–3 + D1/D2 stack (apk 31) held on a 12-phone backpack walk + 2-phone morning walk: max JS-event gap ≤ 6.1 s on every session (10 Android models, Android 8→15, 40–55 min locked-in-pocket), `notifyCount=0` fleet-wide, rail wakes 42–48/walk. See the 2026-06-11 addendum. (History: locked-pocket walks froze the JS event loop 40+ min — ExoPlayer-migration regression, root cause = no in-renderer AudioContext keeping the Chromium renderer scheduled; see the 2026-06-09 addendum fix-plan.)
- **B4 / R27 startup + stalled-signal calibration** — Android startup gate confirmed comfortable on a GOOD-GPS device (8giw: `fixes=2 ready=1 rejected=3`, no stuck startup). **But `nlrc` (Fairphone 4, avgAcc ~21 m) never cleared the gate in 74 min** — `gps_startup_rejected reason=accuracy`/`stale` ×dozens; the then-`STARTUP_FIX_MAX_ACCURACY_M=15` + 2-distinct-fix gate stranded poor-GPS / stationary onboarding at `rdv` (see 2026-06-06 addendum, correction 2). **2026-06-10: the gate is now a flat 20 m bar (no relax) — see the 2026-06-10 addendum.** **2026-06-11: two good-GPS Android devices cleared the 20 m bar in ~10–20 s** (couple of rejections, ready at acc 6 m) — no good-device regression. Confirm the FP4 clears at 20 m at VILLEURBANNE; iOS still needs data. Cosmetic: the evaluator keeps emitting `gps_startup_rejected` after the walk starts (2026-06-11 finding 5, fix A2). **New related blind spot:** the state machine tracks freshness, not accuracy — a fresh-but-200-m-wrong stream reads healthy (2026-06-11 finding 3, fix A4 `degraded` state).
- **E1/E2/E3 zone-overshoot gates** — 300 `accuracy_near_border` events from 8giw (SM-A515F, avgAcc=7m): p75=−3m inside, p95=+3.2m outside. **2026-06-11: +~1 150 events from 2 devices (avgAcc 6–9 m): outside-overshoot p50 ≈ +4–5 m, p95 ≈ +13 m — wider spread than 8giw.** Still high-accuracy devices only; need a session with avgAcc 20–30m before setting production thresholds.
- **H1 ExoPlayer backend validation** — ⚠️ **SECOND DEVICE FAILED** (2026-06-06, `y9ns` SM-A528B / Galaxy A52s 5G / GIVORS_V7_CBR): **38 `step_voice` playerrors** = `MediaCodecAudioRenderer` hardware-decoder exhaustion under instance churn (`format_supported=YES`, transient). The 8giw "0 errors" result was one device + one parcours only. **⚠️ 2026-06-11 (H1-ter): REPRODUCED ON audio-simple 0.3.4** (`1we3`, Sony F5121, Android 8 — 4 errors, 1 voice recovered via C4 retry, 3 ambient layers lost; 11 other Android walks clean). **Same-day reframe: all 4 errors are `native_error_code 5001` = AUDIO_TRACK_INIT_FAILED — likely AudioFlinger *track* exhaustion, not codec exhaustion; the 0.3.4 SW-decoder fix may target the wrong resource** (the y9ns A528B errors share the message shape but predate the code field). **B1 diag SHIPPED in audio-simple v0.3.5** (decoder name + sink trail + instance-population counts on every error; needs apk 32). The A528B itself was a visitor's phone (not fleet) — B3 rescoped to F5121 desk soak + diag-based BYOD coverage. **Howler retirement POSTPONED** until the class is fixed and re-validated. See 2026-06-06 + 2026-06-11 addenda (fix plan round B). **✅ 2026-06-15 (2): B3 soak DONE — 5001 = AudioTrack-pool exhaustion CONFIRMED on 2 F5121 (`bhhk`/`oat9`, apk 34/0.3.5, ~20 errors each, `decoder_software=true`, `players_prepared≈13`); B2b (reactive reclaim+retry, any player) + narrowed B2a (proactive headroom) shipped in audio-simple 0.3.6. **apk-35 re-soak (`mtme`/`wdly`) VALIDATED: 0 audio errors on the F5121 (was 17/20) — proactive cap prevented all 5001s; reactive B2b never needed; 0.3.7 adds proactive telemetry. **2026-06-16 backpack field day (9 walks, 0 audio errors, F5121 clean; cap rides to 11 then sweeps, 1–4×/walk all proactive) — Howler-retirement gate MET (clean re-soak + clean fleet day), retirement now unblocked.** See the [2026-06-15 (2) addendum](#2026-06-15-2-addendum--b-round-5001--audiotrack-exhaustion-confirmed-f5121-sim-soak-b2b--narrowed-b2a-shipped-audio-simple-036).**
...
**Recommended next moves (post-2026-06-07 audit — ⚠️ partially superseded 2026-06-11: Android is on apk 31 and the P0 backpack re-test is DONE; current priorities are the 2026-06-11 fix plan rounds A/B):**
...
- **B4/R27 GPS startup gate — ⚠️ SUPERSEDED 2026-06-10:** this once read "adaptive relax SHIPPED (2026-06-09 #2)", but that relax was found buggy (acceptance widened while the rdv gate's own re-check stayed at the strict 15 m — the two diverged) and **REPLACED with a flat 20 m bar, no relax**. See the [2026-06-10 addendum](#2026-06-10-addendum--full-codebase-review-startup-gate-simplified-to-a-flat-bar-server-auth-hardening). The 20 m bar is the first-pass value to confirm against the FP4 at VILLEURBANNE.
...
3. **✅ SHIPPED [native, power-opt 0.3.2 PO-10] — `WebView.setRendererPriorityPolicy(RENDERER_PRIORITY_IMPORTANT, waivedWhenNotVisible=false)`.** The most *direct* lever: by default WebView drops the renderer to WAIVED priority when not visible → eligible for freeze/kill; `waivedWhenNotVisible=false` keeps it IMPORTANT (un-frozen) while the screen is off. Added as `PowerOptimization.SetRendererPriority(keepWhenHidden)` (UI-thread, API 24+), toggled by `RENDERER_KEEPALIVE.start()/stop()` on the same parcours lifecycle as levers 1–2. Pairs with the audiofocus FG service (process not cached) → renderer stays scheduled. Telemetry: `renderer_priority`. Needs container rebuild (apk 29).
...
5. **Native geofence wake-rail [RESEARCH-FIRST] — PROMOTED to prototype-now by 2026-06-09 D2 (no longer gated on 1–3 failing).** Android `GeofencingClient` registers coarse circular geofences at zone boundaries; a crossing wakes the app via PendingIntent → plugin nudges the renderer / delivers a fix → **JS still does the precise polygon trigger** (wakeup-only, mirrors the iOS rail; audit Decision 2.B). This is the Android analog of the iOS rail — NOT a logic port. Plus 2026-06-09 D1: a native JS-liveness watchdog on the same AlarmManager fires an OS-scheduled notification if JS stops acking — the recovery net if the wake-rail also can't keep JS scheduled.
...
## 2026-06-09 addendum (2) — pre-VILLEURBANNE robustness review (decisions)

Full-codebase review against this audit, challenging implementation choices for a pro-grade locked-pocket UX. The review confirmed every "shipped" claim matches source (no phantom items) and surfaced the gaps below. Five decisions taken with the operator; two shipped immediately (server + doc), three are roadmap commitments. The throughline: **iOS is robust because it has a native OS-level wake mechanism (the CLCircularRegion rail) independent of the JS loop; Android has no such net yet** — every Android resilience claim still routes through a WebView JS loop the OS can freeze. The decisions below close that asymmetry.

### 🟠 D1 — Android native JS-liveness watchdog [IMPLEMENTED in bg-geo v2.15.0 — awaiting apk 30 build] — NEW

**Problem the keepalive levers don't cover:** levers 1–3 (2026-06-09 fix-plan) try to *prevent* the renderer freeze, but if they fail on some OEM the walk dies **silently** — no device-side actor notices or tells the walker. The local-notification chain that could nudge them is disabled (`NOTIF_CHAIN_ENABLED=false`, `pages.js:4092`) **and** re-arms via a JS `setTimeout` (`pages.js:4116`), so it would die with the JS loop anyway. Meanwhile the bg-geo AlarmManager was provably alive throughout the 2026-06-09 freeze (fired 110–154×).

**Decision:** add a native dead-man's-switch in the bg-geo fork, riding the AlarmManager that already survives the freeze. JS calls a native `ackAlive()` on each processed real fix; when the AlarmManager fires during an active walk and sees `now − lastAck > ~90 s`, it posts an **OS-scheduled** (not JS-scheduled) local notification + vibration ("Flânerie s'est mise en pause — touchez pour reprendre"). Tap → app foregrounds → JS resumes → walk catches up. Converts a silent 40-min dead walk into a recoverable blip. Android analog of the iOS rail's `_doForceReacquire`. Needs apk 30 + desk validation (lock 15 min, kill the renderer, confirm the notification fires).

### 🟠 D2 — Android `GeofencingClient` wake-rail [IMPLEMENTED in bg-geo v2.15.0 — awaiting apk 30 build]

Was lever 5 in the 2026-06-09 fix-plan, gated behind a levers-1–3 failure. **Promoted to parallel track.** Rationale: levers 1–2 are circular (Web-Audio keepalive needs the AudioContext to stay scheduled, but the OS can suspend it at lock and `poke()` needs a live JS loop to recover it); only lever 3 (PO-10 renderer priority) acts from native, and `RENDERER_PRIORITY_IMPORTANT` is documented to keep the *process* un-cached — **not** to defeat Chromium's background renderer freeze (a distinct mechanism). So all three levers may land and the walk still freeze. The Android `GeofencingClient` rail is the proven pattern (it is what makes iOS robust) and doesn't depend on the uncertain "Web Audio keeps the renderer scheduled" premise: a coarse circular geofence at each zone boundary wakes the app via PendingIntent → native nudges the renderer / delivers a fix → **JS still does the precise polygon trigger** (wakeup-only, mirrors iOS, Decision 2.B). Prototype in parallel with the levers-1–3 desk test rather than burning a VILLEURBANNE day discovering the levers don't hold. **Still REJECTED:** porting spot.js polygon math + audio orchestration to native (old lever 6) — this is a scheduling problem, not a compute one.

### 🟠 D3 — VILLEURBANNE device list: codec re-test + weak-GPS calibration [COMMITTED, operator]

"Audio clean fleet-wide" on 2026-06-09 is *supporting* evidence, not the owed confirmation — the **SM-A528B / Galaxy A52s 5G** that threw 38 `MediaCodecAudioRenderer` errors on 2026-06-06 was **not in the bag**. Operator will run that exact phone (with `GIVORS_V7_CBR`) at the next field test to finally close P0.1 codec-exhaustion / unblock Howler retirement. Also bring **one genuinely weak-GPS device (Fairphone-4-class, avgAcc ~20–30 m)** to calibrate the startup gate (`nlrc` was stranded 74 min) and collect `accuracy_near_border` at realistic accuracy for E1/E2/E3. Without both devices present, P0.1 and the B4/R27 + E1/E2/E3 calibration stay open regardless of how the day goes.

### 🟢 D4 — Server telemetry ingest hardened against corrupt-file blackhole [SHIPPED, server — `server.js`]

**Found:** `/telemetry-push` did `JSON.parse(fs.readFileSync(...))` **unguarded** then `fs.writeFileSync(filePath, …)` **non-atomically**. A writer crash mid-write (OOM / deploy / power blip) truncates the session file; the next POST then throws 500 on parse, and the client — correctly — retries that session **forever** from its new durable `telemetry_pending` buffer, permanently blackholing the walk and wasting shared-uplink bandwidth. The client-side durability work (#6/#7 above) was undercut by a server that couldn't accept the retry. **Fixed:** (1) the read+parse is wrapped in `try/catch` — on corruption the bad file is quarantined to `<id>.json.corrupt-<ts>` and a fresh session starts, so ingest survives; (2) writes go to `<id>.json.tmp-<pid>-<ts>` then `renameSync` (atomic on POSIX) so readers only ever see a complete file. No rebuild — server-side, reaches all phones on restart. (The `/telemetry/sessions` + `prune-short` readers were already `try/catch`-guarded; this was the one unguarded path.)

### ⏸️ D5 — RESUME cue marked PAUSED (not shipped) [DOC + code comment]

The audit listed `RESUME_PLAYER` (`images/resume.mp3`, the "bon, j'en étais où ?" relaunch cue) under shipped global players and the validation matrix ("Kill mid-step, relaunch → RESUME_PLAYER plays once"). In fact **no `resume.mp3` is bundled** (afterplay/youlost/gpslost exist as real files; resume is absent), so `isLoaded()` is false and the `isLoaded()`-guarded call sites no-op. **Decision: this is a paused feature, not in use yet** — keep the player wired (re-enabling is a one-file drop, no code change) but do not count it as shipped. Code comment added at `pages.js` `RESUME_PLAYER`. (See architecture-summary correction below.)

### D1 + D2 implementation (bg-geo v2.15.0, 2026-06-09) — code complete, needs apk 30

Both shipped to the fork the same day they were approved (operator building apk 30 before the next field test). All native changes are in `cordova-background-geolocation-plugin` (no power-opt change — the renderer nudge uses bg-geo's own cached WebView handle, so it's self-contained); webapp wiring rides the next deploy but is inert until apk 30 exposes the native actions (graceful errback otherwise).

**D1 watchdog — files:**
- `RawLocationProvider.java`: static `sLastJsAckMs` / `sWalkActive` + watchdog counters; `ackAlive()` / `setWalkActive()` / `nudgeRenderer()` / `maybeFireWatchdog()` / `postWatchdogNotification()`. The check rides the existing `LocationWakeReceiver.onReceive` (BG-5 alarm — proven alive during the freeze). **Two-tier:** tier 1 (first stall ≥ `JS_STALL_MS=90s`) re-asserts renderer priority and waits one alarm cycle; tier 2 (sustained, throttled to one per `WATCHDOG_NOTIFY_MIN_GAP_MS=180s`) vibrates (direct `Vibrator` — no permission needed) + posts an `IMPORTANCE_HIGH` heads-up notification whose tap re-launches the app (→ foreground → renderer un-freezes → JS resumes → next `ackAlive` cancels it).
- `NotificationHelper.java`: new `bgloc-watchdog` `IMPORTANCE_HIGH` channel (vibration on).
- **Renderer nudge:** bg-geo's CDV layer caches the Cordova WebView (`RawLocationProvider.setWebView()` in `pluginInitialize`); the background receivers re-assert `setRendererPriorityPolicy(IMPORTANT, false)` from a bare Context. Honest limit: if the renderer is *hard*-frozen despite already being IMPORTANT (PO-10), re-asserting is a no-op — the **notification tap is the guaranteed recovery**; the nudge only catches the soft-throttle case.
- Webapp: `ackAlive()` on every real fix in `geoloc.js _callbackPosition` (Android); `setWalkActive(true/false)` at parcours start / `PAGES_CLEANUP['parcours']`. Telemetry `watchdog_stats` on the 30 s poll (`notifyCount>0` = the net fired = the freeze happened despite levers 1–3).

**D2 wake-rail — files:**
- `GeofenceRailReceiver.java` (new): `GeofencingClient` register/clear (reuses the GMS `play-services-location` dep already pulled by the Fused helper — no new Gradle dep); manifest-registered so a crossing wakes the process from a cached state. `onReceive` → `RawLocationProvider.onRailWake()` → **nudge renderer + run the watchdog check immediately** (a crossing either transparently resumes a soft-throttled renderer or promptly escalates D1 instead of waiting for the slow Doze alarm). **Strictly wakeup-only:** the rail delivers no fix and never triggers audio — the native raw stream is already alive, so the next *real* fix flows into the now-running JS for the polygon trigger (Decisions 1.D / 2.B).
- `plugin.xml`: receiver registration + `ACCESS_BACKGROUND_LOCATION` (already granted by the walk's Always gate).
- Webapp: `pages.js` Android branch reuses `computeGpsRail(PARCOURS)` (identical geometry to the iOS rail) → `configureAndroidRail`; cleared in `PAGES_CLEANUP` + defensively in the provider's `onStop`. Telemetry `gps_rail_configured`/`gps_rail_wake_stats` tagged `platform:'android'`.

**Validation (apk 30):** D1 desk test — reach the parcours page, lock screen ~12 min; if the renderer freezes, expect `watchdog_stats notifyCount≥1` + a vibrating "Flânerie en pause" notification that resumes on tap. D2 — a short locked 2-step walk should show `gps_rail_configured` then `gps_rail_wake_stats wakeCount` climbing with the route advancing without a manual unlock. Both compose with levers 1–3: levers prevent the freeze, D2 re-schedules JS at zone approaches, D1 is the guaranteed-recovery net.

### #2 GPS startup gate — adaptive relax + hard block [⚠️ SUPERSEDED 2026-06-10 — relax was buggy, replaced by a flat 20 m bar; see the 2026-06-10 addendum. The description below is kept for history only — `_effectiveAccuracyBar()`, the `STARTUP_ACCURACY_RELAX_*` constants, and `gps_startup_relaxed` no longer exist in source.]

Root cause confirmed in source: the sole strand was `geoloc.js` `_updateStartupReadiness` rejecting `accuracy > STARTUP_FIX_MAX_ACCURACY_M (15)` **before `fixCount` even increments** — a device whose GPS floor is worse than 15 m (FP4 ≈ 21 m) fails there on every fresh fix, forever (nlrc, 74 min). (The 8 m second-fix requirement is an `OR` with a 5 s time-gap, so stationary onboarding clears *that* fine — not the strand.) The P0.2b 90 s "Démarrer quand même" force-start only papered over it and pushed the call onto a lone visitor.

**Fix (operator-chosen):** the accuracy bar now **self-relaxes** — holds 15 m for `STARTUP_ACCURACY_RELAX_AFTER_MS=30 s`, then `+STARTUP_ACCURACY_RELAX_STEP_M=5 m` per `STARTUP_ACCURACY_RELAX_INTERVAL_MS=20 s`, capped at `STARTUP_ACCURACY_MAX_M=25 m`. The clock anchors only on a *fresh* fix failing on precision (staleness/no-signal never relax — those mean GPS isn't delivering). A ≤25 m device clears at its own floor by ~55–60 s with no walker interaction; good devices are untouched (clear ≤15 m before the clock starts). New `_effectiveAccuracyBar()`; `next.maxAccuracyM` carries the live bar into the rdv label ("à N m max") and `gps_startup_rejected.max_accuracy_m`; `gps_startup_relaxed` logs each upward step.

**Crucially — a device worse than the 25 m ceiling stays BLOCKED, by design.** The "Démarrer quand même" force-start is **retired**: at the 90 s wall the rdv page now shows loan-phone-only guidance ("demandez un téléphone de prêt… la marche ne peut pas démarrer de façon fiable sur cet appareil") and logs `gps_startup_blocked` (replaces `gps_startup_fallback_offered`/`_used`). Operator rationale: a sub-ceiling-GPS phone should walk on a loan device, not walk a bad walk. (Re-enabling a staff override = restore a `#rdv-force-start` click→`PAGE('checkaudio')` handler.) **TEST-FIRST** — watch the FP4 at the next field test (D3) clear at 25 m, and confirm no good device regresses.

### #6 smaller items — dispositions

- **(a) `motionIsStationary` GPS-lost/frozen suppression** — confirmed intended; **left as-is**.
- **(b) iOS B4 watchdog `forceReacquire` cap** — **bumped 3 → 10** (`geoloc.js`) to align with the native rail cap; a 45-min iOS-26 walk with several blackouts can legitimately need more than 3.
- **(c) 6 overlapping keepalive mechanisms** — **deferred**; consolidation pass waits for VILLEURBANNE telemetry showing which levers carry the load (removing any now, pre-field-test, would be reckless).
- **(d) hardcoded SAS `'4321'`** — intended, **kept** (low-stakes gating; per-parcours server codes would be a separate change).

---

## 2026-06-10 addendum — full-codebase review; startup gate simplified to a flat bar; server-auth hardening

Full read-through of the webapp + server + launcher + bg-geo v2.15.0 native against this audit. The review **confirmed every "shipped" claim matches source** (no phantom items) and that the big open risks are execution, not missing code: the Android keepalive stack (levers 1–3 + D1/D2) is well-built but **never run on a device** (the screen-locked desk test, no walk, is the cheapest de-risk), apk 29/30 are unbuilt, and the SM-A528B codec re-test (P0.1) is still owed. One real bug was found and fixed, plus three low-stakes server-auth items.

### 🔴 Bug found — the adaptive relax (2026-06-09 #2) was defeated by the gate's own re-check; **REPLACED with a flat 20 m bar**

The #2 adaptive relax set `startupReadiness.ready = true` (and even emitted `gps_startup_ready`) against the *widening* bar, but the actual rdv gate — `hasFreshFix()` → `GEO.startupReady()` (`geoloc.js`) — ended with an **independent** re-check against the fixed base `this.startupFixMaxAccuracyM = 15`, which the relax never updated. So a 16–25 m device (the FP4 the fix was written for) was reported *ready* by the state machine yet `startupReady()` returned `false` → the gate never opened → it still hit `gps_startup_blocked` at 90 s. Worse, telemetry showed `gps_startup_ready` firing **while the walker was stuck** — a misleading divergence that would have made the VILLEURBANNE weak-GPS test (D3) read as a pass while failing.

**Fix (operator-chosen, webapp-only, no rebuild):** drop the relaxation entirely; use a **single flat 20 m** startup accuracy bar (`STARTUP_FIX_MAX_ACCURACY_M = 20`). This *dissolves* the bug rather than patching it — with one bar, acceptance (`_updateStartupReadiness`) and the gate (`startupReady`) agree by construction, so no path can report ready while the gate stays shut. Removed: `_effectiveAccuracyBar()`, the four `STARTUP_ACCURACY_RELAX_*` constants, the `accuracyRelaxStartMs` tracker field, the relax branch, and the now-dead `gps_startup_relaxed` event. `gps_startup_rejected` no longer carries `base_accuracy_m` (its `max_accuracy_m` is now the flat 20). Behaviour: a device holding ≤ 20 m clears rdv; worse than 20 m stays BLOCKED → loan phone (un-changed operator decision); freshness gate (12 s) + 2-distinct-fixes rule untouched; good devices unaffected. The P0.2b "Démarrer quand même" force-start stays retired (`#rdv-force-start` hidden). **TEST-FIRST** — confirm the FP4-class device at the next field test clears at 20 m and no good device regresses. **This supersedes the 2026-06-09 addendum (2) "#2" section and the line 463 audit finding below — the adaptive-relax machinery they describe no longer exists in source.**

### 🟢 Server-auth hardening [SHIPPED, server — `modules/simpleAuth.js`]

Three low-stakes items found during the review, fixed (no rebuild; reaches all clients on server restart):
- **Open redirect on login** — post-login `next` went to `res.redirect(req.query.next)` unvalidated (`/login?next=https://evil.com` bounced an authenticated user off-site). New `safeNext()` allows only same-origin relative paths (rejects absolute URLs, protocol-relative `//host`, and `/\`).
- **Non-constant-time password compare** — `password === ADMIN_PASSWORD` / `=== guestPwd` replaced with `safeEqual()` (sha256 + `timingSafeEqual`), matching the cookie-verify path already in the file.
- **Cookie missing `secure`** — the login cookie now sets `secure` when the request arrived over HTTPS (`req.secure || x-forwarded-proto: https`), so production gets it without breaking local HTTP testing.

### Review notes (no action taken — for the record)

- **Telemetry at-least-once duplication** — a flush the server *received* but never ACKed (app died mid-POST) is re-sent on next launch from `telemetry_pending` and the server appends without dedupe (`server.js`). Deliberate durability trade-off, not a regression; analyzer should expect occasional duplicate events. Cheap future mitigation: a client-side per-event seq + server-side dedupe.
- **`RENDERER_KEEPALIVE` coexistence (lever 1)** — still the one unvalidated coexistence risk (Web-Audio silent loop at `volume 1.0` concurrent with native ExoPlayer narration). The desk test must watch that narration is not ducked/interrupted.
- **`fs.watch(..., {recursive:true})` in `updater.js`** — recursive watch is Node-20+ on Linux; the running deploy proves it's fine, noted only so a Node downgrade isn't done blind.

### 🟠 D1/keepalive hardening #1–#4 (2026-06-10 review of the locked-pocket stack) [IMPLEMENTED — bg-geo v2.15.1-pending + webapp; needs apk 30]

Focused review of the Android JS+GPS+audio keepalive stack surfaced four gaps; all four fixed same day. Native parts ride the same apk 30 build as D1/D2; webapp parts ride the next deploy (inert until apk 30 exposes them, graceful errback otherwise).

1. **#1 — D1 disarmed itself on process death.** `sWalkActive`/`sLastJsAckMs` were in-memory statics, so an OEM kill mid-walk (Samsung "put to sleep", HyperOS) restarted the START_STICKY service with the watchdog **disarmed** — the walk died silently, the exact failure D1 exists to report. Fix (`RawLocationProvider.java`): walk gate persisted to `SharedPreferences` (`bgloc_watchdog/walkActive`); `restoreWalkStateIfNeeded()` re-arms on provider `onStart` after a service rebirth (primes the ack clock → stale acks escalate normally), and the manifest-registered `GeofenceRailReceiver` restores too — if it reanimated a **headless** process (provider not yet running = the WebView is gone for certain) it posts the recovery notification immediately, at the zone boundary where the walker is missing audio. Plus a new cap `WATCHDOG_MAX_NOTIFY_PER_STALL=3` (consecutive tier-2 notifications without an intervening ack) so an abandoned walk can't vibrate the walker every 3 min forever. Telemetry: `watchdog_stats.walkActiveRestored` + `.notifySinceAck`.
2. **#2 — ack conflated "JS alive" with "GPS delivering".** Acks fired only on real fixes, so a ≥2 min GPS blackout (urban canyon) with a perfectly live JS loop would have escalated to vibration + "Flânerie en pause" (tap fixes nothing; meanwhile the webapp's own `gps_lost` UX is already handling it). Fix (`geoloc.js`): a ~25 s timer ack on the existing 1 s `stateUpdateTimer` (Chromium intensive throttling clamps background timers to 1/min — still inside the 90 s stall bar); per-fix acks kept as belt-and-braces.
3. **#3 — stronger nudge + direct renderer-liveness probe.** `nudgeRenderer()` now also calls `WebView.onResume()` + `resumeTimers()` (legal anytime, no-op when resumed). New `sendRendererProbe()`: `evaluateJavascript("1", cb)` from the alarm tick — an answered previous probe **proves** the renderer runs JS (→ tier 2 suppressed: it's a GPS-side stall, not a freeze); a previous probe left unanswered a full alarm cycle (or no WebView to probe — reborn process) **confirms** the freeze. GPS-independent, queue-on-frozen semantics make it the definitive frozen/alive signal. Telemetry: `watchdog_stats.probeSentCount/.probeAnswerCount/.lastProbeAnswerAgeMs`.
4. **#4 — lever 1's `poke()` was a single shot.** It ran only on `visibilitychange`; if Chromium suspended the AudioContext *after* the hidden event, nothing retried (the acknowledged lever-1 circularity). Now also poked per processed real fix in `_callbackPosition` (bridge callbacks keep flowing during the soft-throttle phase when JS timers already stall — the best recovery window before the hard freeze) and on the 25 s timer tick. Found while wiring: `poke()` after `stop()` would have silently **restarted** the keepalive post-walk (stop() doesn't null the howl) — `RENDERER_KEEPALIVE` now has an `_active` lifecycle gate.

Validation rides the existing apk-30 desk test (addendum (2) D1/D2): additionally kill the app process mid-walk (`adb shell am kill`) and expect the rail crossing or service rebirth to produce the recovery notification (`walkActiveRestored:true` in `watchdog_stats` after relaunch); and a simulated GPS blackout (walk into a basement / GPS off) must NOT produce a notification (probe answers, tier 2 suppressed).

### 🟠 #5 — lever 1 asset hardened: silent clip → inaudible 20 Hz tone [SHIPPED, webapp asset — rides next deploy]

The Howler era was protected by *real narration* through the in-renderer AudioContext, not just the silent loop — and modern Chromium classifies a page as audible by **measuring output power** (`AudioStreamMonitor`, floor ≈ −72 dBFS), so a digitally-silent clip may hold the AudioContext open yet never earn the audio exemption from background freezing/throttling. `images/flanerie.mp3` replaced: **20 Hz sine, 10.0 s loop (200 exact cycles), MP3 CBR 128 kbps** (old silent file kept as `images/flanerie-silent-orig.mp3`). Source level −30 dBFS; LAME's psychoacoustic model eats ~21 dB at 20 Hz → **decoded −51.5 dBFS peak** (measured): ~20 dB above Chromium's audibility floor, yet ~27 dB *below* the human absolute threshold at 20 Hz (~75 dB SPL) even at max volume, before earbud/speaker rolloff (steep under 80 Hz). Same file feeds the native `SILENT_PLAYER` (ExoPlayer) — equally inaudible there. No code change; `RENDERER_KEEPALIVE` telemetry unchanged. Desk-test cross-check: if the renderer still freezes with `renderer_keepalive` running, the probe counters (#3) now say so definitively.

### 🟠 A — D1 tier 1.5: auto-foreground recovery, loan fleet [IMPLEMENTED — bg-geo + power-opt 0.3.3-pending (PO-11) + webapp; needs apk 30]

Tier 2's weakness: it needs a human to feel a vibration through a pocket and act. With the **"Display over other apps"** grant (the documented background-activity-launch exemption), the watchdog can recover **without the walker**: when the freeze is confirmed (probe unanswered) it launches `WatchdogRecoveryActivity` — fullscreen black, minimum brightness, shown OVER the keyguard *without dismissing it* (pocket touches reach nothing), `turnScreenOn` — whose visibility lifts the process out of the cached tier and thaws the renderer. The activity polls `sLastJsAckMs` and finishes the moment JS acks (or after 12 s), screen back off. Walker experience: a brief glow in the pocket, audio resumes itself. Escalation: freeze → auto-fg attempt at ~2 min → retry next alarm cycle → notification (cap `WATCHDOG_MAX_AUTOFG_PER_STALL=2`, acks reset it; the reborn-headless case never routes here — no WebView to thaw — and goes straight to the notification).

- **bg-geo:** `WatchdogRecoveryActivity.java` (new), `tryAutoForeground()` in `maybeFireWatchdog` between probe-gate and notification; activity + `SYSTEM_ALERT_WINDOW` declared in `plugin.xml`. `watchdog_stats` adds `autoForegroundCount` / `lastAutoForegroundAgeMs` / `autoForegroundLastRecoveryMs` (>0 = ms-to-ack, −1 = attempted-no-recovery).
- **power-opt PO-11:** `IsOverlayGranted` / `RequestOverlay` (opens the per-app Settings screen — Android has no runtime dialog for this permission). iOS stubs return granted/no-op.
- **webapp:** devmode tools page "Affichage par-dessus (prêt)" button — reads state on entry, opens Settings, re-reads on return (`overlay_grant_state` / `overlay_grant_requested` telemetry); `power_state_at_parcours` now carries `overlay_granted` so every walk records whether auto-recovery was armed.
- **Provisioning:** grant once per loan device via the devmode button (or `adb shell appops set <pkg> SYSTEM_ALERT_WINDOW allow` in bulk). **Never requested from visitors** — BYOD keeps the notification path; the feature self-gates on `Settings.canDrawOverlays()`.
- **Validation (apk 30, loan phone with grant):** desk freeze test → expect screen to glow black ~2 min in, `autoForegroundCount≥1` with `autoForegroundLastRecoveryMs>0`, **no notification**; revoke the grant → same test must fall back to vibration + notification.

---

## 2026-06-11 addendum — two-wave field day (TOUSSIEU/FLANERIE_INVITES_V3): Android JS-suspension P0 VALIDATED at scale; 4 new findings + fix plan

Two waves on FLANERIE_INVITES_V3 (21 steps), apk 31 (Android) / apk 28 (iOS), webapp `64a0bcb`, bg-geo 2.15.1 / power-opt 0.3.3 / audio-simple 0.3.4: **13:26** = 2 phones carried by one walker (Pixel 4a `7jir`, SM-G960U1 `hxgs`); **16:30** = 12-phone backpack (10 Android models Android 8→15 + iPhone 8 iOS 16.7.10 + iPhone SE 3 iOS 26.4.2). **All 14 walks completed** (the one telemetry-incomplete session is an upload failure, not a walk failure — see finding 2).

### ✅ P0 — Android background WebView JS-suspension: VALIDATED, CLOSED

The launch blocker is dead. Every phone spent 40–55 min locked/backgrounded; **max gap between consecutive JS-emitted events was ≤ 6.1 s on 13/14 sessions** (~8 100 events each; the one 42.9 s gap is `u7wb`'s crash-relaunch, below). Pre-fix this scenario froze JS 40+ min. Levers confirmed live in-session: `audio_context_state running` (lever 1), `renderer_priority {keep:true, result:"kept"}` (lever 3). **D1 watchdog clean across the fleet**: `notifyCount=0` everywhere (zero false fires — the #2/#3 hardening holds), `autoForegroundCount=0` (never needed), renderer nudges ~1/min. **D2 Android rail live**: `gps_rail_configured` + 42–48 `gps_rail_wake` per walk. Architecture D dedupe correct on GMS devices (`fusedSuppressed=1332` on `hxgs`); `fusedAvailable=false` on the Pixel 4a is **explained: it runs LineageOS** (no GMS Fused provider) — fail-soft raw-only carried the walk as designed, not a bug.

Also banked: morning startup gate cleared in ~10–20 s on both good-GPS devices (flat 20 m bar, final acc 6 m — FP4-class still untested); ~1 150 fresh `accuracy_near_border` events for E1/E2/E3 (outside-overshoot p95 ≈ +13 m on avgAcc 6–9 m devices — wider than 8giw's +3.2 m); morning `user_lost` at step 17 on both phones = real walker detour, lost→map→recovered UX worked end-to-end; `u7wb` (T433D) had 1 mid-walk crash with a textbook resume (43 s, step 12 @ seek 39.9 s) — device watch-list, not a code item.

### Findings (numbered for the fix plan)

1. **H1-ter — ExoPlayer `MediaCodecAudioRenderer` exhaustion REPRODUCED on audio-simple 0.3.4** (`1we3`, Sony F5121, Android 8 — second affected device after SM-A528B). 4 playerrors: BLOC_10 voice (recovered via C4 retry ~20 s later), BLOC_14 zone music + BLOC_14 afterplay (afterplay fell back to default via loaderror path), BLOC_18 zone music. Walker heard all narration but lost 3 ambient layers. **The 0.3.4 SW-preferred-decoder fix did not eliminate the class.** Howler stays the default fallback.
2. **Wi-Fi-only loan phones lose walk telemetry irrecoverably** (`fkpp`, Sony F5121 `57faadd7`, `overlay_granted=false`). Uploads stopped at walk-minute 18 (left Wi-Fi range); the walk itself ran perfectly to completion — proven by the cumulative native watchdog counters in the same-process 17:27 re-arm session (`notifyCount=0`, nudges 8→40 across the dark period = JS alive throughout). The telemetry buffer is in-memory only and a closed session's failed final flush is never retried, so ~30 min of events were lost at re-arm. Operational note: that device is confirmed Wi-Fi-only — any Wi-Fi-only loaner currently reports nothing once out of range.
3. **iOS fix-QUALITY collapse is invisible to the signal-state machine** (`4och`, iPhone 8, iOS 16.7.10). Minutes 11–19: fixes kept arriving (fresh, so state read `ok`/brief-`frozen`) but accuracy collapsed to p50=40 m / p95=200 m and position diverged up to 137 m from backpack-mates → **zones 2–3 never fired, steps 2–3 unheard**; the walker's manual unlock at 18.5 min forced reacquisition and a skip-burst to step 4. B4/R27 watch freshness, not accuracy — a fresh-but-200-m-wrong stream reads as healthy and the walker gets no prompt. (Old 2017 hardware buried under 11 phones = aggravating test artifact, but the blind spot is real.)
4. **`gps_recovered` vibrates for episodes the walker was never told about** (`0ay2`, iPhone SE 3). 6 pocket vibrations = the unconditional `navigator.vibrate([200])` in the `gps_recovered` handler firing after silent ~30 s freezes that never reached escalation (by design, the 2026-06-10 #3 debounce). Recovery feedback for an invisible event is pure noise through a pocket.
5. **Startup-gate evaluator keeps emitting after walk start** (`hxgs`: `gps_startup_rejected` ×5 at walk-minutes 4–26 whenever accuracy brushed 21–26 m). Cosmetic telemetry noise; pollutes B4 calibration queries.

### Fix plan (proposed 2026-06-11)

**Round A — webapp-only, one deploy, no rebuild [IMPLEMENTED 2026-06-11, unverified on device]:**
- **A1 [SAFE-TODAY] (finding 4)** — gate the `gps_recovered` vibration on "the episode was walker-visible": set a flag where the walker is actually disrupted (`gps_frozen_escalated`, `gps_lost`, `gps_revoked` handlers — the `pauseAllPlayers()` sites), vibrate-on-recover only when set, clear after. Telemetry unchanged.
- **A2 [SAFE-TODAY] (finding 5)** — stop startup-gate evaluation once the walk session opens (or once `gps_startup_ready` has fired and rdv passed): guard `_updateStartupReadiness`'s rejected-emit on pre-walk phase.
- **A3 [SAFE-TODAY] (finding 2)** — telemetry persistence: on flush failure, spill the unflushed buffer to localStorage (capped, FIFO-evict oldest, per-session key); on app launch and on `session_start`, upload any spilled batches before live traffic; while a session is open, retry on `online` event. Server already accepts append-by-sessionId, so replays are idempotent-enough; D4 corrupt-file hardening covers truncated uploads. Validation: airplane-mode mid-walk segment → events arrive after reconnection; re-arm before reconnection → events arrive on next launch.
- **A4 [TEST-FIRST, telemetry-only first] (finding 3)** — new `degraded` signal state: sustained accuracy collapse (first-pass: `acc > 35 m` sustained > 60 s while `!motionIsStationary`) emits `gps_degraded` / recovery emits into the existing `gps_recovered` flow. **Phase 1 = telemetry-only** (no walker-facing change) to size the problem at VILLEURBANNE; phase 2 (conditional on data) reuses the frozen-escalation UX (pause + jingle + overlay "signal GPS imprécis"). Thresholds from `4och`: normal walk p50 ≈ 10 m, collapse p50 ≈ 40 m — 35 m splits them cleanly.

**Round B — native, audio-simple, container rebuild (finding 1, H1-ter):**

**⚠️ REFRAMED (2026-06-11, same day): the failure class is probably NOT codec exhaustion.** All 4 F5121 errors carry `native_error_code: 5001` = `ERROR_CODE_AUDIO_TRACK_INIT_FAILED` — the decoder was fine; the renderer could not create an **AudioTrack** (AudioFlinger's mixer-track pool, a *different* finite resource the 0.3.4 SW-decoder preference does not touch; a prepared-but-paused ExoPlayer still holds a track). The archived `y9ns` (SM-A528B) errors predate the `native_error_code` field but share the identical message shape (`MediaCodecAudioRenderer error … format_supported=YES`) — plausibly the same 5001 class, meaning 0.3.4 may have been aimed at the wrong resource from the start. B1's diag decides this per error.

- **B1 [✅ IMPLEMENTED 2026-06-11 — audio-simple v0.3.5 (`a3e9a6a`), pushed + reinstalled into FlanerieCordova; needs apk 32 build].** Verified `setEnableDecoderFallback(true)` + the SW-preferred `MediaCodecSelector` are present (they are, since 0.3.4). New: a per-instance `AnalyticsListener` captures the **selected decoder name** (`onAudioDecoderInitialized`; `decoder_init_count > 1` = fallback/re-init engaged) and **non-fatal `onAudioSinkError`s** (these precede a fatal 5001); every loaderror/playerror payload now carries a `diag` object — `decoder`, `decoder_software` (omx.google./c2.android. heuristic), init/sink counters, plus plugin-wide `players_total` / `players_prepared` / `players_playing` at error time (the service silent player is a constant +1 on top). Webapp `player.js` `_logAudioTelemetry` spreads `diag` into the `audio_playerror`/`audio_loaderror` telemetry events. Reading the next failure: `decoder_software=true` + 5001 + high `players_prepared` ⇒ AudioTrack exhaustion (→ B2b); `decoder_software=false` or 4xxx decoder-init codes ⇒ the codec path after all.
- **B2 [TEST-FIRST — decide after first B1 data; two shapes]:** **B2b (preferred first, small surface)** — reactive pressure-relief valve: on a 5001, natively stop prepared-but-not-playing **loop** players (frees their AudioTracks; `play()` already re-prepares from `STATE_IDLE`, so JS semantics survive; never touches non-loop voice positions or the silent keepalive), then re-prepare + retry the errored player once — it only acts in a path that today loses an audio layer anyway. **B2a (structural, only if B1 shows the population is chronically high)** — eager release of past-step players (`step_past_unload` hook exists) / small shared pool for loops; bigger regression surface: start latency, the A8/A8b cold-load race class prewarm exists to prevent, crossfade pair-sizing (Ambiance needs outgoing+incoming live simultaneously), and JS `PAUSED_PLAYERS`/`DUCKED_PLAYERS` bookkeeping assumptions.
- **B3 [RESCOPED 2026-06-11]** — the SM-A528B was a Givors **visitor's own phone**, not fleet: direct re-test impossible. The in-house F5121 is now the repro device (it failed in the field on 0.3.4). Ladder: (1) F5121 desk soak — drive a parcours in simulate mode (devmode) to reproduce the load churn until a 5001 fires *with diag attached*; (2) fix per B1 data (likely B2b); (3) F5121 clean soak + one clean fleet field day; (4) BYOD coverage comes from the diag itself — any future visitor-phone error reports decoder + population, so the A528B class is diagnosable without the device. The D3 "SM-A528B codec re-test" commitment is satisfied by (1)+(4). Howler retirement stays gated on (3).

**Not fixing:** iPhone 8/iOS 16 as a loaner stays accept-or-retire (operational call — its GPS hardware is the weak link); T433D on the crash watch-list; HTC U11 was an external tester's phone, excluded.

**Round A implementation notes (2026-06-11, same day):**
- **A1** — `GPSSIGNAL_DISRUPTED` module flag in `pages.js`, set at the four `pauseAllPlayers()` disruption sites (`gps_lost`, frozen-escalation timer, `showGpsRevokedOverlay`, `showBatteryKillOverlay`); the `gps_recovered` vibrate is gated on it and the event now carries `was_disrupted`.
- **A2** — `_updateStartupReadiness` no longer emits `gps_startup_rejected` when `tracker.ready` (the gate never un-readies; rejection state updates continue, only the emit is gated).
- **A3** — three changes in `telemetry.js`: (1) `end()` no longer deletes `telemetry_pending` and fires a blind `sendBeacon` — it persists the buffer FIRST then runs a **confirmed** `flush()` (success clears PENDING, failure keeps it under the original session id via a `ctx` snapshot threaded through `_persistPending`/the flush catch — the exact `fkpp` hole: a failed final flush used to fall into the no-session branch and delete the durable copy it was protecting). (2) `online` event listener: live session → `flush()`; no session → `_drainPendingNow()` (drain extracted from the once-per-launch wrapper, chunked at `DRAIN_CHUNK=200` events/POST with a resumable remainder rewrite, `savedAt` slot-ownership guard kept). (3) buffer-cap eviction is now type-aware (`_capBuffer`): high-frequency diagnostics (`voice_snapshot`, `route_probe`, `gps`, stats…) evict first so the walk narrative (steps, errors, lifecycle) survives a long offline stretch; the flush catch also refuses to inject a dead session's events into a NEW session's buffer.
- **A4** — `_trackAccuracyDegradation` in `geoloc.js`, called from `_callbackPosition` on real fixes only; emits `gps_degraded` / `gps_degraded_recovered` (no walker-facing change). `session.mjs` prints `degraded=N/rec=N`; `analyze.mjs` flags `gpsDegraded>=1`.
- **Validation (next field day):** A1 — pocketed walk with brief freezes → zero recovery buzzes, `gps_recovered was_disrupted=false`; one real lost episode → buzz on recovery. A2 — zero `gps_startup_rejected` after `session_start` in walk sessions. A3 — airplane-mode mid-walk segment → events arrive after reconnection (`pending drain OK`, chunked); end the walk offline + re-arm → events arrive on next launch under the original session id. A4 — `gps_degraded` count on a weak-GPS device; tune `DEGRADED_ACCURACY_M=35` / `DEGRADED_SUSTAIN_MS=60s` from the histogram.
...
## 2026-06-15 (2) addendum — B-round: 5001 = AudioTrack exhaustion CONFIRMED (F5121 sim soak); B2b + narrowed B2a shipped (audio-simple 0.3.6)

The B3 soak (operator ran 3 simulated walks) closed the codec-vs-AudioTrack question and shipped the fix the same day. **apk 34** correctly carries **audio-simple 0.3.5** (the B1 diag — the 0.3.5 reinstall from the earlier 2026-06-15 session took in this build, resolving correction #2), so every error arrived with full diag.

### ✅ 5001 reproduced deterministically, with diag — root cause PROVEN

Sim drive of FLANERIE_INVITES_V3 (21 steps), apk 34 / audio-simple 0.3.5, `source:'simulate'` ×1000s, all completed:

| Session | Device | OS | playerrors | verdict |
|---|---|---|---|---|
| `20260615_185912_bhhk` | **Sony F5121** | A8 | **17** | reproduced |
| `20260615_185913_oat9` | **Sony F5121** | A8 | **20** | reproduced |
| `20260615_191317_noic` | **TCL T433D** | A14 | **0** | clean |

B1 diag identical across **100%** of the F5121 errors: **`native_error_code: 5001`** (`ERROR_CODE_AUDIO_TRACK_INIT_FAILED`), **`decoder_software: true` / `OMX.google.mp3.decoder`** (decoder fine), **`players_prepared: 13` while `players_playing: 2–4`**, `sink_error_count: 2`. → **AudioFlinger track-pool exhaustion, NOT codec exhaustion** — confirms the 2026-06-11 reframe and proves the 0.3.4 SW-decoder preference was aimed at the wrong resource. The TCL (A14, larger pool) never errored. (`noic`'s "6× 5001" are inside `route_probe` GPS events — a coincidental numeric value, not audio.)

**Why 13 prepared:** past-step players are *already* released on next-step fire (`spot.js` B1 unload, line ~712); the survivors holding tracks are prepared-but-idle **loops** (zone/object/afterplay), the prewarmed next step, and the persistent global cue players — each in `STATE_READY` holds an AudioTrack (`pause()` keeps it; only `stop()`→IDLE frees it).

**Walker impact:** errors hit **ambient/music loop layers only** (e.g. `…BLOC_04…_MUSIC.mp3`); **`step_voice_failed: 0`** (narration intact) but **`audio_playerror_retry: 0`** — looped layers get no recovery today (C4 retry is voice-only), so a 5001 = a silently dropped layer, ~20×/walk on the F5121.

### ✅ Fix shipped — audio-simple 0.3.6 (fork commit `9842473`)

**B2b [reactive, native, any player type].** On a 5001 in `ExoPlayerInstance.onPlayerError`: `plugin.reclaimIdleTracks(handle)` **silently** `stop()`s every idle reclaimable instance (→ STATE_IDLE, frees its AudioTrack), then re-prepares + retries the errored instance once (`RECLAIM_RETRY_DELAY_MS=80`, restoring `intendedPlayWhenReady`). Fires for **any** layer (voice/zone/object/afterplay), not just voices — the layers that were dropped silently now recover. Guard `trackReclaimRetries≤1`/episode (reset when `onIsPlayingChanged` confirms audio flows); if the retry also 5001s, the real `playerror` is emitted and the existing JS fallback applies.

**Narrowed B2a [proactive, same sweep].** `maybeReclaimForHeadroom()` runs before any instance acquires a track in `load`/`play`/`prewarm`: if `preparedCount() ≥ PREPARED_SOFT_CAP (11)`, shed idle tracks first — keeping the pool under the F5121's ~13 ceiling so a 5001 rarely happens at all. One tunable; no change to zone-retention or prewarm logic (the structural eager-release B2a stays deferred).

**Invisible to JS (the load-bearing property):** the reclaim is a *silent* `player.stop()` (no event), so `PlayerSimple.isLoaded()` (a `_loadError`-only flag, [player.js:1175](www/app/assets/player.js#L1175)) stays true and `spot.js` will not reload + re-grab the freed track; the shim `_loaded` mirror is untouched; `play()` self-heals by re-preparing from IDLE. Skips: playing layers, players buffering-toward-play (`intendedPlayWhenReady`), and paused voices holding a mid-narration position (`RECLAIM_KEEP_POSITION_MS=1500`). Loops restart from 0 (ambient — harmless).

**Telemetry:** new `audio_track_reclaim` event (native `trackreclaim` → shim passthrough → `player.js` logs `reclaimed`, `players_prepared/playing/total`, `decoder_software`, `error_code`). **The native fix functions without the webapp change** — the reclaim+retry happen natively regardless; the webapp event is observability only.

**Files:** [ExoPlayerInstance.java](../cordova-plugin-audio-simple/src/android/ExoPlayerInstance.java) (5001 branch, `reclaimIfIdle`/`retryAfterTrackReclaim`/`maybeReclaimForHeadroom`/`emitTrackReclaim`, `intendedPlayWhenReady`/`trackReclaimRetries`), [AudioSimplePlugin.java](../cordova-plugin-audio-simple/src/android/AudioSimplePlugin.java) (`reclaimIdleTracks`/`preparedCount`), [www/AudioSimple.js](../cordova-plugin-audio-simple/www/AudioSimple.js) (`trackreclaim` passthrough) — fork → **0.3.6**; [www/app/assets/player.js](www/app/assets/player.js) (`audio_track_reclaim` hook) — webapp.

**Deploy (2026-06-15, in progress):** 0.3.6 released via the `plugin-upgrade` skill (commit `9842473` pushed; reinstalled into FlanerieCordova, `cordova prepare`; native `ping()` patched to 0.3.6 → `session_diag` will read `audio-simple@0.3.6`). Operator building **apk 35** + deploying the webapp now.

### Validation owed (apk 35 re-soak — same two F5121)
Sim-drive all 21 steps and expect: `audio_playerror`/5001 → **~0**; `audio_track_reclaim` with `reclaimed > 0`; `players_prepared` capped near 11. **If the population still rides the cap**, add the deferred **structural B2a** (eager release of past-step/zone/global players). **Howler retirement stays gated** on a clean re-soak + one clean fleet field day. `PREPARED_SOFT_CAP=11` is tuned to the F5121's observed ~13 ceiling — re-tune from the re-soak.

### ✅ Re-soak result (apk 35, audio-simple 0.3.6, `mtme`/`wdly`) — FIX VALIDATED

Two F5121 sim walks (21 steps, both completed). On the exact device + parcours that deterministically threw 17 / 20 `audio_playerror`/5001 on apk 34: **0 `audio_playerror`, 0 `audio_loaderror`, 0 `step_voice_failed`, 0 retries/fallbacks** on both. (The "5001" substrings that remain are inside `gps_quality_summary` / `route_probe` — GPS numerics, not audio.)

**Mechanism = prevention, not recovery.** `audio_track_reclaim` was also **0** → the reactive B2b path never fired (no 5001 reached the error handler). The proactive narrowed B2a (`maybeReclaimForHeadroom`, cap 11) kept the prepared-AudioTrack population under the ~13 ceiling, so exhaustion never occurred. Indirect footprint: `audio_load_ready` slightly exceeds `audio_uri_resolved` (mtme +1, wdly +5) = a handful of reclaim→re-prepare cycles, not thrashing.

**Two caveats:** (1) the proactive path emitted no telemetry, so the capped population couldn't be measured directly (0 errors/reclaims ⇒ no `players_prepared` in the file); (2) the reactive B2b path is **unexercised on-device** (prevention was total).

**→ audio-simple 0.3.7** (fork commit, released via `plugin-upgrade`; **apk 36 owed**): `maybeReclaimForHeadroom` now emits `audio_track_reclaim {proactive:true, reclaimed, players_prepared, players_playing}` when the cap sweep frees a track — so the next run shows the cap doing its job and the live headroom (and whether the structural B2a is ever needed: population pinned at the cap with frequent reclaims ⇒ yes; rare ⇒ no). Reactive 5001 recoveries now carry `proactive:false`.

**Net:** the codec/AudioTrack B-round is **functionally closed** (worst-case device clean across two walks). Howler retirement stays gated on one clean **fleet field day** (a real walk, not sim) made measurable by the 0.3.7 proactive telemetry.

---

## 2026-06-16 addendum — backpack multi-device field day (apk 36 / audio-simple 0.3.7): audio fix + media downloader VALIDATED in the wild; Howler-retirement gate MET

First **real-GPS** field run of the current build. **One operator carrying ~8 phones in a backpack** (same methodology as the 2026-06-09 / 06-11 backpack days — **NOT** 8 independent visitors), plus a dawn staff test, on FLANERIE_INVITES_V3 (21 steps). **9 phone-walks, all completed (step 20), 0 failures, 0 audio errors.** All Android (no iOS exercised today). 8 walks on **apk 36 / audio-simple 0.3.7**; the dawn HTC test on apk 33. Devices: HTC U11 (A8, dawn), Redmi Note 10 / M2101K7AG (A11) ×2, SM-A415F (A12), **F5121 (A8 — the audio-issue class)**, SM-A515F (A13) ×4.

### ✅ Audio fix validated in the wild + first field measurement of the cap
- **0 `audio_playerror`, 0 reactive reclaims** (B2b never fired) across all 9 walks; **the F5121 completed clean** on 0.3.7.
- The proactive cap (narrowed B2a) does continuous real work: `audio_track_reclaim {proactive:true}` fires **1–4×/walk on every device class**, and `players_prepared(post) + reclaimed ≈ 11` every time (0+11, 2+9, 3+8, 4+7) — the population rides up to the cap, the sweep clears it to the playing set (2–4), repeat. **The cap is essential** (these devices are genuinely under AudioTrack pressure — the F5121 would 5001 without it) **and sufficient** (0 errors).
- **Structural B2a stays DEFERRED.** Only residual signal: `players_total` climbed to **38** on the F5121 (instances accumulate while tracks stay capped) — no failure today; that's the inefficiency eager-release would tidy if memory pressure ever surfaces on low-RAM devices.

### ✅ Media downloader validated on a genuinely slow link
First real on-site downloads: **`uckx` = 6m31s** for the full 176 MB / 52-file pack (~450 KB/s) and `xk88` = 24 s; the rest sub-1.5 s cached re-uses. **Both real downloads: `retried:0`, and ZERO `media_download_partial` all day.** The 6.5-min slow download is the proof — the old 12 s whole-transfer timeout would have aborted the 12.4 MB Alex_secours file; the chunked Range + resume + 20 s stall-watchdog rode it. **The 2026-06-12 downloader rewrite is field-closed.**

### ✅ Startup gate — no stranding
Every phone cleared the flat-20 m bar (1–4 normal warmup rejections); one transient `blocked` (`eo3w`) recovered immediately. Départ GPS adequate (avgAcc ~9.5 m even on the F5121). The weak-GPS-at-départ risk did not materialize — **but this venue had good GPS, so the weak-GPS gates (flat-20 m under sustained ≥20 m, A4, E1/E2/E3) remain technically uncalibrated.**

### ✅ GPS + keepalive — flawless
`gps_frozen / lost / degraded = 0` on all 9 walks; `notifyCount=0`, `autoForegroundCount=0`, rail wakes 38–44/walk. The Android background-survival stack held across the run.

### Two benign blemishes
- **HTC `p4hs`** — an accidental mid-walk app-kill by the operator (**not** a crash/bug): D1 watchdog fired (`notify=3`) + resume recovered → completed step 20, 0 voice errors. A free real-world validation of crash-resume + watchdog on a genuine process death.
- **F5121 `ytq9`** — one 345 s GPS gap in the final ~6 min; steps 14–17 skip-completed during it (likely some missed narration on those blocks); `gps_frozen/lost` did not fire (keepalive masked the stale stream — the documented blackout pattern; A4 is accuracy-based, not gap-based). Single old device, end of walk, completed. Because the backpack carried all phones on the same path, this is a **device-hardware difference** (the F5121's GPS dropped where the others held), not a route problem.

### Roadmap impact
- **Howler-retirement gate MET.** The audio B-round needed *clean re-soak + one clean fleet field day*; the 06-15 re-soak (`mtme`/`wdly`) + today's backpack run (9 walks, 0 audio errors on exoplayer across 8 device classes) satisfy both. Retirement is unblocked — a deliberate "remove the Howler fallback" call, not forced.
- **Media downloader field-validation: DONE** (closes the last open item from the 2026-06-12 addendum).
- **Caveats:** backpack ≠ independent-visitor scale, and all-Android / no-iOS today. Real BYOD-visitor behaviour and iOS at the départ are still only inferred.

---

## 2026-06-19 addendum — public field day (FLANERIE_INVITES, apk 36): Android background JS-suspension P0 REGRESSED on the loaner fleet; fix plan

First true public day (~35 visitor walks, staggered group start ~14:00, apk 36 / webapp `82ae260` / `68a9db0b`). Clean on the headline axes — **0 `step_voice` errors, 0 audiofocus failures, no build/parcours skew, no real GPS blackout on the BYOD fleet**, ~34 walks reached step 20. Two real walker-facing issues: an A54 (`qhkz`, SM-A546V) that **crash-looped at step 1** (OEM kill outran progress, 4 relaunches, never advanced — new watch-list device); and an iPhone 11 (`fgl3`, iOS 16.1.2) blocked by **Motion = Denied** (`auth_status:2` on all 63 prompts — *not* the §-bug-2 NotDetermined structural hang; the hard motion gate then re-prompted to no effect since iOS won't re-present a denied prompt). Recovered by a loaner. Full write-up + per-session tables: [telemetry/reports/2026-06-19-field-day.md](telemetry/reports/2026-06-19-field-day.md).

**The structural finding is a P0 regression.** Three loaner-fleet walks (`z23h`, `bt1t` = SM-A515F; `gjtm` = SM-A415F) render with **no GPS track + grey steps** in the control-panel viewer. Drill-down on `z23h` (SM-A515F, 47 min, "step 20"):

- `gps` telemetry: **17 events, all in the first 1.5 min** (foreground, at the rdv), then nothing. `route_probe` (the step engine's position eval) **stops at 2 min**. `location_dispatch_stats` / `gps_quality_summary` / `accuracy_near_border`: all silent 1.5 → 46 min.
- **Native stayed alive the entire walk:** `location_dispatch_stats.rawDelivered` climbs **1160 → 3797** (~2,600 fixes delivered) and `gps_rail_wake_stats.wakeCount` **0 → 41** across the silent window; `bg_geo_stop` is `intentional:true` at 45.5 min (the last-step GPS cutoff). bg-geo ran fine end-to-end.
- So **the renderer's JS event loop was starved ~44 min** while native bg-geo + native ExoPlayer audio kept running. The walk's steps did **not** advance from GPS after 2 min — the **native audio chain auto-advanced** block→liaison→block to "step 20." Completion here is **audio-chained, not GPS-validated** (the walker may have walked the route; telemetry can't confirm it for those 44 min).

**The dead path is entirely the JS renderer:** `bgGeo.on('location')` (`geoloc.js:1543`) → `positionCallback` → `_callbackPosition` → `emit('position')` (`:1009`, feeds the step engine) **and** `TELEMETRY.gps()` (`:1058`). When the renderer suspends, none of it runs; native audio (ExoPlayer) and the AlarmManager rail are unaffected, so the walk looks fine to the walker and "complete" to the analyzer.

**Why it slipped the 2026-06-11 "P0 VALIDATED & CLOSED":**
- The keepalive that keeps the renderer scheduled is the **in-renderer Web Audio context** (lever 1). These walks play narration through **native ExoPlayer** (audio-simple), so if the in-renderer AudioContext isn't actively running for the whole walk, nothing holds the renderer awake. The apk-31 validation fleet likely didn't exercise this device × native-audio combination.
- **The D1 native JS-liveness watchdog did not recover it.** `ackAlive` (`:1045`) stops when JS is starved, which should escalate the AlarmManager watchdog — yet `z23h` ran 44 min starved with no recovery. Either D1 didn't fire, or its renderer re-poke is ignored on these devices.
- Concentrated on the **loaner/spare fleet** (SM-A515F, SM-A415F, M2101K7AG). The Xiaomi M2101K7AG shows a *milder* variant (`as7n`/`q7a5`: a log gap that **resumes**, so the track still spans the route) — same class, less severe.

### Fix plan

**Phase 0 — diagnose the exact break (before any fix).** The two candidate causes (renderer suspended vs bridge-callback starved) need different fixes.
- Emit a **native-vs-JS reconciliation** field on each `location_dispatch_stats` tick: native `rawDelivered` delta vs JS-processed (`gps` logged / `route_probe` run) since the last tick — one event then shows "native N, JS M" instead of having to infer the gap post-hoc.
- Log the in-renderer keepalive **`AudioContext.state`** for the walk's duration, and increment a counter *inside* `bgGeo.on('location')` (`:1543`) flushed via native snapshot — distinguishes "renderer asleep" from "renderer awake, bridge starved."
- **Repro** on an SM-A515F: a locked-pocket walk with native-ExoPlayer narration (the path the apk-31 fleet may have missed).

**Phase 1 — keep the renderer alive on the native-audio path (the actual fix).**
- Guarantee the **in-renderer 20 Hz inaudible keepalive tone runs for the whole active walk**, decoupled from the voice player's lifecycle (today it may run only while a Web Audio clip plays; native-ExoPlayer narration leaves it idle). This is the documented root-cause class (ExoPlayer-migration regression).
- Confirm **`KeepRunning` + power-opt renderer-priority (PO-10)** are actually applied on these models; capture `standby_bucket`/priority at walk start (some OEMs ignore the hint).

**Phase 2 — make D1 actually recover this.**
- Investigate why the AlarmManager watchdog didn't escalate during `z23h`'s 44-min starve. Extend recovery to force `forceReacquire()` + a JS `getCurrentLocation` drain on wake, not just a renderer re-poke.

**Phase 3 — don't lose the track even if JS is starved (cheap mitigation, ship now).**
- On `visibilitychange`/wake, **drain the native backlog**: read the retained `lastDelivered*` / dispatch counters and emit a synthetic `gps` catch-up sample so the track + blackout scan aren't blind. Mirror the iOS catch-up-burst on Android.

**Phase 4 — fix completion semantics (analysis, ship now).**
- `analyze.mjs`/`isCompleted`: `maxStep≥20` reached with a **multi-minute `route_probe` gap** (or native `rawDelivered` climbing while JS-processed = 0) is **audio-chained, not GPS-validated** → a distinct `audio-completed (GPS-suspended)` bucket, not "completed." Key the check on JS-suspension, **not** on logged-fix span — span gives false negatives precisely on these sessions (it fooled the first pass of this very report).

**Phase 5 — validate & re-open.**
- Loaner backpack walk; confirm the JS-event gap is back under the apk-31 acceptance bar (≤ ~6 s) on SM-A515F/A415F with native-audio narration.
- This addendum **re-opens the Android JS-suspension P0** as a device-gap regression (loaner fleet + native-ExoPlayer audio path); the 2026-06-11 "CLOSED" holds only for the validated fleet/audio path.
...
### GPS / lifecycle
`session_start`, `session_resume`, `session_restart_click`, `session_end`, `session_diag`, `parcours_restore`, `parcours_freshness_check`, `parcours_update_chosen`, `bg_geo_authorization`, `app_visibility`, `gps_lost`, `gps_recovered`, `gps_callback_gap`, `gps_state`, `gps_startup_fix`, `gps_startup_ready`, `gps_startup_rejected` (carries `max_accuracy_m` = the flat 20 m bar; **2026-06-10:** `base_accuracy_m` dropped and `gps_startup_relaxed` removed — the adaptive relax was replaced by a flat bar), `gps_startup_blocked` (rdv wall after 90 s when the device can't hold a fix at the 20 m bar: loan-phone guidance, no force-start; replaces the retired `gps_startup_fallback_offered`/`_used`), `real_callback_freshness` (30 s, includes `cl_state` + `ios_stream_health` on iOS and `alarm_wake_stats` + `location_dispatch_stats` on Android), `ios_power_state` (60 s iOS), `bg_restrictions_recheck` (5 min Android, includes `memory_info` + `standby_bucket`), `power_state_at_parcours` (now includes `auto_revoke_whitelisted` on Android), `alarm_wake_stats` (30 s Android, bg-geo v2.8.0 P0.5 Fix 1e diagnostic), `location_dispatch_stats` (30 s Android, bg-geo v2.9.0 Architecture D: `{fusedAvailable, rawDelivered, rawKeepalive, fusedDelivered, fusedSuppressed, fusedStaleIgnored, lastDeliveredSource}`), `gps_quality_summary` (flush summary including `freshSamples`, `staleSamples`, `startupGradeSamples`). Each `bg-geo` location event now carries `dispatch_source` ∈ `{raw, raw-keepalive, fused}` and `is_keepalive` on Android (`is_keepalive` already on iOS via F-G4). **2026-06-09:** `gps_frozen` (frozen-state entry) + `gps_frozen_escalated` (only after `GPS_FROZEN_SUSTAIN_MS` of sustained frozen — the deferred-escalation debounce, #3); `renderer_keepalive` / `renderer_keepalive_error` (Android in-renderer Web Audio keepalive, lever 1) and `renderer_priority` / `renderer_priority_error` (native WebView renderer-priority toggle, lever 3). **2026-06-11 A4:** `gps_degraded` / `gps_degraded_recovered` (sustained accuracy collapse on real fixes — `acc > 35 m` for ≥ 60 s; telemetry-only, fresh-but-wrong streams the freshness states can't see); `gps_recovered` now carries `was_disrupted` (A1 — whether the episode had walker-visible escalation). **2026-06-09 D1/D2 (bg-geo v2.15.0):** `watchdog_stats` (30 s Android: `{walkActive, lastAckAgeMs, notifyCount, rendererNudgeCount}` — `notifyCount>0` means the JS-liveness net fired during a freeze; **2026-06-10 #1–#3 adds** `probeSentCount`/`probeAnswerCount`/`lastProbeAnswerAgeMs` (renderer-liveness probe), `notifySinceAck` (tier-2 cap counter), `walkActiveRestored` (walk gate re-armed after a process kill); **tier 1.5 adds** `autoForegroundCount`/`lastAutoForegroundAgeMs`/`autoForegroundLastRecoveryMs`), `overlay_grant_state` / `overlay_grant_requested` (devmode loan-phone setup flow; `power_state_at_parcours` also carries `overlay_granted`); `gps_rail_wake_stats` (30 s Android: `{wakeCount, lastWakeAgeMs, lastRegionId, lastTransition}` — geofence rail crossings); `gps_rail_configured` / `gps_rail_cleared` now also fire on Android (`platform:'android'`).

### Step / parcours
`step_fire`, `step_done`, `step_skip_done`, `step_implicit_done`, `step_audio_trigger` (carries `accuracy`, `consecutive_inside_samples`, `time_since_first_inside_ms`, `neighbor_distances`, `step_fire_latency_ms`), `step_resume_current`, `step_past_unload`, `step_voice_failed`, `step_afterplay_fallback`, `step_prewarm_next`, `accuracy_near_border` (when within 20 m), `voice_snapshot`, `voice_snapshot_skipped`, `user_lost`, `user_recovered`.

**2026-06-16 telemetry slimming (Phase A, webapp — `parcours.js`):** `route_probe` payload slimmed to `{currentStep, gpsAccuracy, source, acceptedForTrigger}` — the 7 distance/border/next-step fields fired per GPS fix (~2840×/walk) but were read by **no** analyzer (every consumer reads only `currentStep`), so dropping them cuts ~66% of route_probe's bytes (it was **~50%** of a session file). **`parcours_store` event removed** — a strict subset of `voice_snapshot` (same `store()` call, fewer fields), read by no consumer; the `localStorage` persistence is unchanged. Net per-session ≈ **−37%**, with analyzer + control-page output byte-for-byte unchanged. **Phase B** (downsampling `route_probe` / `voice_snapshot` *frequency* to on-change + 30 s heartbeat, → ≈ −80% overall) is **deferred until after the Fri/Sat public session** to keep full diagnostic resolution for the first public run.

### Audio
`audio_play_requested`, `audio_play_started` (carries `load_duration_ms`), `audio_play_gate`, `audio_play_timeout`, `audio_play_stuck`, `audio_play_stuck_retry`, `audio_play_timeout_self_healed`, `audio_loaderror`, `audio_playerror` (both carry `error_type` ∈ {not_found, network, decode_failed, src_unsupported, timeout, stuck} **and `native_error_code`** = the raw Media3 `PlaybackException.errorCode` on ExoPlayer, so DECODER_INIT_FAILED vs DECODING_RESOURCES_RECLAIMED is distinguishable — the mapped 1–4 `error_code` collapses both to 4/`src_unsupported`, which masked the 2026-06-06 codec-exhaustion failure), `audio_uri_resolved`, `audio_playerror_retry`, `audio_engine_reset`, `audio_engine_reset_error`, `audiofocus_request_fail`, `audiofocus_keepalive_started`, `audiofocus_session_released`, `audio_route_changed`, `audio_session_state` (60 s). **Round 21:** `audio_uri_resolved` and `audio_loaderror`/`audio_playerror` now carry `backend` ∈ {`exoplayer`, `howler`, `howler-fallback`, `native`} so analyze.mjs can bucket post-rollout comparisons cleanly. On the ExoPlayer path the `error_code` 1–4 is derived natively from `PlaybackException.errorCode` (see `ExoPlayerInstance.mapToHowlerCode` — IO range → 2/4, parsing/decoder → 3, unknown → 4) so `classifyAudioErrorType()` in player.js produces the same `error_type` enum without changes. **2026-06-15 (2) B2b (audio-simple 0.3.6):** `audio_track_reclaim` — a 5001 (`ERROR_CODE_AUDIO_TRACK_INIT_FAILED`) triggered the native AudioTrack reclaim+retry; carries `reclaimed`, `players_prepared`/`players_playing`/`players_total` (post-reclaim), `decoder_software`, `error_code`. Its presence (instead of an `audio_playerror`) means the layer was recovered, not dropped. Android-only; never on Howler/iOS (the JS hook is gated to the `exoplayer`/`audio-simple` backends — Howler's `.on()` throws on unknown events). **0.3.7:** also fires from the proactive headroom sweep (narrowed B2a) with `proactive:true` (no `error_code`) so the cap and live headroom are measurable even when no 5001 ever occurs; reactive 5001 recoveries carry `proactive:false`.

### Operator / rearm
`rearm_button`, `rearm_pre_state`, `walk_end_shutdown`, `inter_session_idle_ms` (on `session_start`), `exoplayer_release_all` / `exoplayer_release_all_error` (Round 21; emitted before `audiofocus_session_released` in both walk-end A1 and rearm A3 paths so the ExoPlayer FG service ID 7375 tears down ahead of AudioFocusService ID 7374). **2026-06-09:** `loan_auto_marked` (`is_loan` auto-set on first devmode entry, #1 — replaces the manual toggle); `checkaudio_retry` + enriched `checkaudio_fail`/`checkaudio_play_ok` (now carry `backend` + `native_error_code`, #4). The walk-telemetry buffer is now persisted across app death (`localStorage` `telemetry_pending`) and re-sent on the next launch (#6), and flush POSTs are bounded by `POST_TIMEOUT_MS` (#7).
...
- **G2** power-opt v0.3.2: PO-1 LeTV intent fix, PO-2 `GetLastExitReasons`, PO-3 `GetMemoryInfo`, PO-4 `GetStandbyBucket`, PO-5 JSON booleans, PO-6 iOS stub, PO-7 Xiaomi MIUI autostart, PO-8 `skipProtectedAppCheck` guard, **PO-9 `IsAutoRevokeWhitelisted` + `RequestAutoRevokeWhitelist` (v0.3.1)**, **PO-10 `SetRendererPriority` — keep the WebView renderer at IMPORTANT priority when hidden, Android JS-suspension keepalive lever 3 (v0.3.2, 2026-06-09)**, **PO-11 `IsOverlayGranted`/`RequestOverlay` — "Display over other apps" grant state + Settings deep-link for the D1 tier-1.5 auto-foreground (loan fleet, v0.3.3-pending, 2026-06-10)**.
...
- **P3.5c** Resume restores ambiance but **not the current-step voice** (2026-06-19 user report) — ⚠️ **OPEN.** On app/walk resume the ambiance catches back but the current step's narration often does not; only the *next* step continues the voice. **Mechanism (code-confirmed asymmetry):** zone players (`Ambiance`/`Objet`) `resume()` **unconditionally on every position fix while `inside`** (`spot.js:415–431`), so ambiance self-heals from GPS; the step voice has **no per-position auto-resume** — it restores only via the **zone-gated** re-fire (`spot.js:680`, needs `!_active && near && inside`) or the paused-resume (`spot.js:647`, needs `isPaused()`), and the saved seek `resumeStepVoicePos` is consumed **only inside that gated fire** (`spot.js:766`). If the walker has stepped **out of the current step's (small) zone** by relaunch time — or the voice's native track was **reclaimed** during an audiofocus pause (`resumeAllPlayers` resume() then no-ops; the AudioTrack/codec-exhaustion class, see H1/B-round) — neither path fires and the voice stays silent until the next step's fresh fire. **Evidence:** code asymmetry is clear; **2026-06-19 telemetry does NOT show a frequent crash-resume failure** — the voice re-fired on **18/18** clean `done=False` resumes, 7 were resume-at-already-done (correctly skip to next), and the only 2 misses are both the JS-suspended `8naa` HTC U11 (unreliable). So the failure is the **out-of-zone subset** and/or the **soft audiofocus-reclaim** path, neither captured cleanly today. **Plan:** (1) **probe** — emit `resume_voice_recovery {resume_step, done, inside_current_zone, player_state, recovered}` so the next field day splits in-zone vs out-of-zone recovery directly; (2) **repro** — background/kill mid-narration, walk ~20–40 m past the step zone, reopen → expect ambiance back, voice silent until the next step; (3) **fix** — on resume with `resume_step_index=N, done=False`, replay step N's voice from `resumeStepVoicePos` **regardless of current zone presence** (it *is* the current step the walker was inside), and on `AUDIOFOCUS_GAIN` **validate/reload** the voice track instead of a bare `resume()`. Extends P3.5; see the [2026-06-19 addendum](#2026-06-19-addendum--public-field-day-flanerie_invites-apk-36-android-background-js-suspension-p0-regressed-on-the-loaner-fleet-fix-plan).
