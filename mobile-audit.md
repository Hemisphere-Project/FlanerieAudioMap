...
- **B4/R27 GPS startup gate — adaptive relax SHIPPED (2026-06-09 #2), tuning still gates on field data:** the fixed `STARTUP_FIX_MAX_ACCURACY_M=15` strand is fixed — the bar now self-relaxes 15 → 25 m over ~50 s and a sub-ceiling device stays blocked → loan phone (the P0.2b force-start is retired). See the 2026-06-09 addendum (2) "#2" section. The relax schedule + 25 m ceiling are first-pass values to confirm against the FP4 at VILLEURBANNE.
...
### #2 GPS startup gate — adaptive relax + hard block [SHIPPED, webapp] (resolves the review's #2)
