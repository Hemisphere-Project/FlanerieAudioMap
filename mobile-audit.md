...
- **B1 [RESEARCH-FIRST]** — instrument first: log the *selected decoder name* + whether fallback engaged on every ExoPlayer error (the 0.3.4 errors don't say whether the SW preference was even honored on this device class). Verify `setEnableDecoderFallback(true)` is set on the renderers.
- **B2 [TEST-FIRST]** — attack instance churn: cap live ExoPlayer instances (release players for past steps eagerly — `step_past_unload` already exists as the hook), and/or move zone/afterplay loops to a small shared-player pool so codec count stays bounded regardless of parcours size.
- **B3** — re-test ladder: desk soak on the in-house F5121 (cheapest repro, Android 8) → SM-A528B re-test (D3 commitment) → only then reopen the Howler-retirement question.
