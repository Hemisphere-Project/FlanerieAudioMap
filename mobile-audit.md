...
| iOS native plan (H/I/J/K/L) | Plan in [ios-native-plan.md](ios-native-plan.md). **K (native step-state cache) + J (MPNowPlayingInfo + locked remote commands)** shipped in Round 22 (audiofocus v1.8.0), migrated to audio-simple in Round 25. **H (rail of wake-up regions)** shipped in Round 23 (bg-geo v2.10.0 — BG-11, transition-midpoint CLCircularRegions, throttle 3→10, 30 s stall gate). **I (native audio engine + plugin rename)** — phase 1 shipped in Round 24 (plugin rename `cordova-plugin-exoplayer-simple` → `cordova-plugin-audio-simple` @ 0.2.0); phase 2 shipped in Round 25 (audio-simple v0.3.0: iOS AVAudioPlayer pool + AVAudioSession singleton + MPNowPlayingInfo + step-state cache; audiofocus iOS shrunk to v1.9.0; `AUDIO_BACKEND_IOS='audio-simple'` default). **L (CLMonitor iOS 17+)** — scoped down to visit events only; shipped in Round 26 (bg-geo v2.11.0 — BG-12 `startMonitoringVisits` via legacy CLLocationManagerDelegate, no Swift required). CLMonitor proper deferred indefinitely — legacy region API works and visit data is available via the existing path. **All planned iOS workstreams now shipped.** |
