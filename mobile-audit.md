...
- **🟠 P0 — Android background WebView JS-suspension (NEW 2026-06-09; FIX SHIPPED, awaiting validation).** Locked-pocket Android walks freeze the WebView JS event loop for 40+ min (zone-triggering + audio selection + telemetry-flush all stall) while the native bg-geo stream stays fully alive — confirmed across Samsung/Sony/Xiaomi/Fairphone, Android 8→15, battery-opt exempt. Root cause: the ExoPlayer/audio-simple migration moved the silent keepalive native, so no in-renderer AudioContext keeps the Chromium renderer scheduled. **Fix shipped** = keepalive levers 1–3 (see 2026-06-09 addendum fix-plan): in-renderer Web Audio loop [webapp, live], `KeepRunning=true` [container], power-opt 0.3.2 `setRendererPriorityPolicy(IMPORTANT, waivedWhenNotVisible=false)` [native]. **Still needs apk 29 build (levers 2–3 are native) + a backpack re-test** to confirm the JS loop stays alive locked-in-pocket. Likely Android launch blocker until validated.
- **B4 / R27 startup + stalled-signal calibration** — Android startup gate confirmed comfortable on a GOOD-GPS device (8giw: `fixes=2 ready=1 rejected=3`, no stuck startup). **But `nlrc` (Fairphone 4, avgAcc ~21 m) never cleared the gate in 74 min** — `gps_startup_rejected reason=accuracy`/`stale` ×dozens; the then-`STARTUP_FIX_MAX_ACCURACY_M=15` + 2-distinct-fix gate stranded poor-GPS / stationary onboarding at `rdv` (see 2026-06-06 addendum, correction 2). **2026-06-10: the gate is now a flat 20 m bar (no relax) — see the 2026-06-10 addendum.** Confirm the FP4 clears at 20 m at VILLEURBANNE; iOS still needs data.
