...
| iOS native plan (H/I/J/K/L) | Plan in [ios-native-plan.md](ios-native-plan.md). **K (native step-state cache) + J (MPNowPlayingInfo + locked remote commands)** shipped in Round 22 (audiofocus v1.8.0). **H (rail of wake-up regions)**, **I (native audio engine + plugin rename)**, **L (CLMonitor iOS 17+)** still to ship before VILLEURBANNE per agreed sequencing. |
